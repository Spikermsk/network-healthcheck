...
device = []
try:
    resp= get(api="host") # The response (result) from "GET /network-device" request
    status = resp.status_code
    response_json = resp.json() # Get the json-encoded content from response
    device = response_json["response"] # Network-device
except:
    print ("Something wrong, cannot get host information")
    sys.exit()
...
for item in device:
    i+=1
    device_list.append([i,item['hostIp'], item["connectedNetworkDeviceName"], item["id"]])
# Show all network devices under this DNAC's management
# Pretty print tabular data, needs 'tabulate' module
print (tabulate(device_list, headers=['number','ip','Name'],tablefmt="rst"),'\n')
...
try:
    resp = get(api="host/"+id+"/location")
    status = resp.status_code
except:
    print ("Something wrong with GET network-device/"+id+"/location !\n")
    sys.exit()
try:
    ...
    if status == 204:
        print ("No Content in response of GET /network-device/id/location !")
    else:
        print ("Something wrong in response of GET /network-device/id/location!\n")
